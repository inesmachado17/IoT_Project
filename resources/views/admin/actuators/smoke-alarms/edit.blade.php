...
<div class="row d-flex justify-content-center align-items-center">
    <div>
        <form action="/actuators/smoke-alarms/{{ $smokeAlarm->id }}" method="POST">
            @csrf()
            @method('PUT')
            <div class="mb-3">
                <label for="name" class="form-label">Designação</label>
                <input type="text" class="form-control" id="name" name="name" value="{{ $smokeAlarm->name }}">
            </div>

            <div class="mb-3">
                <label for="setting" class="form-label" style="display: block;">Fumo %</label>
                <div class="d-flex align-items-center justify-content-between">
                    <input type="text" class="form-control" id="setting" name="setting"
                        value="{{ $smokeAlarm->setting }}">
                </div>
            </div>

            <div class="mb-3">
                <span id="span-switch-op" class="form-label" style="display: block;">{{ $smokeAlarm->automatic ? 'Automático' : 'Manual' }}</span>
                <div class="d-flex align-items-center justify-content-center">
                    <button id="button-switch-op" type="button" onclick="toggleOperationMode()" class="text-secondary"></button>
                    <input type="hidden" id="automatic" name="automatic" value="{{ $smokeAlarm->automatic }}">
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="d-flex align-items-center justify-content-center">
                    ...
                    <input type="hidden" id="state" name="state" value="{{ $smokeAlarm->state }}">
                </div>
            </div>

            <div class="form-buttons">
                <button type="submit" class="btn btn-primary">Guardar</button>
                <a class="btn btn-outline-secondary" href="/actuators/smoke-alarms" role="button">Cancelar</a>
            </div>
        </form>
    </div>
</div>
